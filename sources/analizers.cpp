...
void processing_filename(const std::string &filename,//первым аргументом принимает имя. полное имя файла без расширения
...
//итератор - некая обертка над каждым элементом массива, позволяет работать с ним удобнее
  ...
  for (const auto &entry : iteratorB{broker_directory}) { //проходим по директориям итератор проходит по файлам. //читаем массив имен файлов в директории и по массиву имен итерируем
    ...
    processing_filename(entry.path().filename().stem().string(), files,//алгоритм по обработке имени файла(передаем имя файла без расширения, преобразуем его в string)
                        accounts);// по ссылке передаем массив файлов и множество указателей
  }
  return broker(name, files, accounts);//В целом: по всей директории обходим каждый файл, формируем контейнеры(43-44) из них создаем брокера и возвращаем его обратно
}
...
std::vector<broker> analyse_all(const std::string &path) { //передается путь в виде строки
  ...
  for (const auto &entry : iteratorB{brokers_directory}) {//Этим циклом мы проходимся по папкам с брокерами
    ...
    if (!brok.files().empty() && !brok.accounts().empty())//Проверка, чтобы за брокеров не считались папки, в которых ничего нет (проверка на неправильных брокеров). Файлы не должны быть пустые и не должны быть пустыми аккаунты
